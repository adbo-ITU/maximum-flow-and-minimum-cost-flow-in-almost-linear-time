...
def assert_valid_solution(
    graph: Graph, s: int, t: int, flow: list[int], flow_value: int
):
    ...
    ), f"Expected max flow {actual_max_flow}, got {flow_value}"
    ...
    assert (
        flow_from_s == flow_value
    ), f"Flow from s = {flow_from_s}, expected {flow_value}"
    assert (
        flow_from_s == flow_to_t
    ), f"Flow conservation not satisfied: flow from s = {flow_from_s}, flow to t = {flow_to_t}"


def assert_flow_is_valid(
    edges: list[tuple[int, int]], capacities: list[int], flow: list[int], s: int, t: int
):
    ...
    for e, (u, v) in enumerate(edges):
        ...
        assert (
            flow[e] <= capacities[e]
        ), f"Flow {flow[e]} on edge {u} -> {v} exceeds capacity {capacities[e]}"
    ...
    for v, f in vertex_flows.items():
        if v == s:
            assert f == -sum(
                flow[e] for e, (u, _) in enumerate(edges) if u == s
            ), "Flow from s does not match"
        elif v == t:
            assert f == sum(
                flow[e] for e, (_, v) in enumerate(edges) if v == t
            ), "Flow to t does not match"
        else:
